...
first_frame=10;
last_frame=100;
values=0; %% 1 to view the plot, 0 not to view the plot
%%
pixel_size=0.16;%% um
exposure_time=0.1;
...
% last_movie=20;
%%
key_word='20180223mray100ms1';
folder_name='/Users/chen/Desktop/data';
d3=dir(folder_name);
for i=3:1:length(d3)
    if contains(d3(i).name,key_word)==1
       sub_folder=d3(i).name;
       folder=[folder_name '/' sub_folder]
          PALM2Dtracking_cellselection4(folder)  %% select cells
          PALM2Dtracking_celloutline3(folder)    %% select outline
    end
end
...
for i=3:1:length(d3)
    if contains(d3(i).name,key_word)==1 
     ...
     last_movie=pp-2
                  PALM2Dtracking_spotfinding4(folder,first_frame,last_frame,pixel_size,values)  %% find spots
